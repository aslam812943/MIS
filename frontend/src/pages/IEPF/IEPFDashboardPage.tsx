  ...
  const fetchDashboard = async (showToast = false) => {
    ...
    try {
      const [metrics, claimsList] = await Promise.all([
        iepfService.getDashboardData(),
        iepfService.getClaims({ status: undefined })
      ]);
    ...
    <DashboardLayout>
      <div className="mis-page mis-animate-in max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div>
            <h1 className="mis-page-title mis-page-title-accent">IEPF Department Analytics</h1>
            <p className="mis-page-desc">Overview of claim files processing efficiency, resolution metrics, and hold reasons.</p>
          </div>
          <button
            type="button"
            onClick={() => fetchDashboard(true)}
            className="mis-btn mis-btn-ghost w-fit self-end sm:self-auto"
          >
            🔄 Refresh Metrics
          </button>
        </header>
